...
10. If the browser console shows `Content-Security-Policy-Report-Only` warnings that mention `connect-src 'none'` or block same-origin frontend assets, inspect the live Cloudflare response headers first. The checked-in nginx config already serves an enforcing same-origin CSP, so a stricter report-only policy on the public hostname usually means Cloudflare or another edge layer is injecting conflicting headers. The repo intentionally still allows inline styles on frontend routes because `keycloak-js` uses hidden iframes with inline `display:none` styling for silent SSO and login-session checks.
11. Keep `STUDYVAULT_AUTH_NAV_RATE` and `STUDYVAULT_AUTH_NAV_BURST` high enough for normal Keycloak account-console redirects on the public hostname. Over-throttling `/realms/studyvault/protocol/openid-connect/auth` can surface as repeated redirects followed by `429 Too Many Requests` when users open `Manage Account` or `Change Password`.
